...
> **2026-09-15:** branch **`feat/broadcast-email-ics`** is built and green but **NOT merged and NOT
> deployed** — see the block directly below. Broadcast email, the event-reminder button and `.ics`
> calendar feeds. Gate: typecheck ✓ / lint ✓ / **1000 tests** ✓ / build ✓. What it needs is the
> signed-in browser walkthrough listed in that block; every admin has TOTP, so only a human with
> the authenticator can do it.
>
